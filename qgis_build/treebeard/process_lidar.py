...
def process_canopy_areas(canopy_gdf, study_area, output_path, buffer_distance=5):
    ...
    try:
        ...
        print(f"An error occurred while processing and exporting data for {study_area['Proj_ID'].iloc[0]}: {e}")
        return None, None
...
def process_lidar_to_canopy(proj_area, las_folder_path, canopy_height=5):
    ...
    try:
        ...
        print(f"An error occurred while processing LIDAR for {proj_area['Proj_ID'].iloc[0]}: {e}")
        return None
